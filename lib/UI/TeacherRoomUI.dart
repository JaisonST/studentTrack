...
class TeacherRoomUI extends StatefulWidget {
  final String schoolDB;
  final List<dynamic> roomList;
  TeacherRoomUI({@required this.schoolDB, this.roomList});

  @override
  _TeacherRoomUIState createState() => _TeacherRoomUIState();
}

class _TeacherRoomUIState extends State<TeacherRoomUI> {
  int liveCases = 0;
  int cap = 0;
  String val;
  bool initial = false;
  ...
  @override
  initState() {
    if (widget.roomList.length != 0) {
      val = widget.roomList[0];
      _items = buildItems(widget.roomList);
